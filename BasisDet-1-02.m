...
PreTreatment[] := 
  ...
  If[Coefficient[Dim, \[Epsilon]] == 0, ExtraDimensionFlag = 0, ExtraDimensionFlag = 1];
  xtable = 
   Table[ToExpression["x" <> ToString[i] <> ToString[j]], {i, 1, L}, {j, 1, physicalDimension}];
  mutable = {};
  If[ExtraDimensionFlag == 1, 
    mutable = 
      Table[ToExpression["\[Mu]" <> ToString[i] <> ToString[j]], {i, 1, L}, {j, 1, L}];
    For[i = 1, i <= L, i++, 
      For[j = 1, j < i, j++, 
        mutable[[i, j]] = mutable[[j, i]];];];];
  ...
  AdditionalKinematics1 = 
    Table[ExternalMomentaList[[i]]*SpuriousMomentaList[[j]] -> 0, 
      {i, 1, Length[ExternalMomentaList]}, 
      {j, 1, Length[SpuriousMomentaList]}] 
      // Flatten;
  AdditionalKinematics2 = 
    Table[SpuriousMomentaList[[i]]*SpuriousMomentaList[[j]] -> 0, 
      {i, 1, Length[SpuriousMomentaList]}, 
      {j, i + 1, Length[SpuriousMomentaList]}] 
      // Flatten;
  (*This is the scalar product list for all vectors in the spacetime basis*)
  KinematicsCondition = 
    Join[Kinematics, AdditionalKinematics1, AdditionalKinematics2];
  Gram = Table[
    SpacetimeBasis[[i]]*SpacetimeBasis[[j]], 
    {i, 1, Length[SpacetimeBasis]}, 
    {j, 1, Length[SpacetimeBasis]}];
  ExtendGram = 
    Table[RemindingMomentaList[[i]]*SpacetimeBasis[[j]], 
      {i, 1, Length[RemindingMomentaList]}, 
      {j, 1, Length[SpacetimeBasis]}];
  ...
  If[Length[RemindingMomentaList] == 1, 
    ExtendGram = {-Sum[Gram[[j]], {j, 1, physicalDimension - NSpurious}] 
                 // Simplify}];
  ...
  ScalarProduct = 
    Table[MomentaProjection[[i]].Inverse[Gram].MomentaProjection[[j]] //Simplify, 
      {i, 1, Length[TotalMomentaList]}, 
      {j, 1, Length[TotalMomentaList]}];
  If[ExtraDimensionFlag == 1, {Print["Extra dimension is turned on."];
    For[i = 1, i <= L, i++, 
      For[j = 1, j <= L, j++, 
        ScalarProduct[[i, j]] -= mutable[[i, j]];];];}];
  ScalarProductRules = 
    Table[TotalMomentaList[[i]]*TotalMomentaList[[j]] -> ScalarProduct[[i, j]], 
      {i, 1, Length[TotalMomentaList]}, 
      {j, i, Length[TotalMomentaList]}];
  ScalarProductRules = Flatten[ScalarProductRules];
  SPList = 
    Join[Flatten[Reverse[Transpose[xtable]]], Union[Flatten[mutable]]];
...
(*Generate the set of cut equations in ALL scalar products.op=0,numeric.op=1,analytic*)

CutGenerator[op_] := 
  Module[{
    option = op, 
    num}, 
  If[option == 0, num = numeric, num = {}];
  CutEqn = 
    Table[Expand[Props[[i]]^2] //. ScalarProductRules // Simplify, 
      {i, 1, Length[Props]}];
  CutEqn = 
    Table[RemoveCoefficient[CutEqn[[i]] //. num, SPList] // Simplify, 
      {i, 1, Length[CutEqn]}];]
...
LinearReduce[ideal1_, Varlist1_] := 
  ...
  PreGr = 
    GroebnerBasis[
      ideal, 
      Varlist, 
      MonomialOrder -> PolyOrder, 
      CoefficientDomain -> RationalFunctions
    ];
  PreLTList = 
   Table[MonomialList[PreGr[[i]], Varlist, PolyOrder][[1]], 
     {i, 1, Length[PreGr]}];
  RSP = {};
  For[i = 1, i <= Length[PreLTList], i++, expr = PreLTList[[i]];
  j = Total[Exponent[expr, Varlist]];
  If[j == 1, RSP = Append[RSP, ExpRecover[Exponent[expr, Varlist], Varlist]]];];
  RSPSolution = 
    Table[RSP[[i]] -> PolynomialReduce[RSP[[i]], 
      PreGr, Varlist, MonomialOrder -> PolyOrder][[2]] 
      // Simplify, 
      {i, 1, Length[RSP]}];
  reducedIdeal = Union[ideal //. RSPSolution // Simplify];
  (*Sort the ISP*)
  ISP = MonomialList[Total[Complement[Varlist, RSP]], Varlist, PolyOrder];
  If[ISP == {0}, ISP = {}];
  reducedIdeal = 
    Table[RemoveCoefficient[reducedIdeal[[i]], ISP] 
    // Simplify, 
    {i, 1, Length[reducedIdeal]}];
...
ISPTreatment[] := 
  Module[{
    ...
    komega, 
    tempPowerList}, 
    {ISP, RSPSolution, CutEqnISP} = LinearReduce[CutEqn, SPList];
  len = Length[ISP];
  ...
  For[i = 1, i <= L, i++, tempPowerList = Table[0, {i, 1, L}];
    ...
    For[j = 1, j <= Length[RenormalizationLoopMomenta], j++, 
      If[RenormalizationLoopMomenta[[j]] == tempPowerList, 
        ...
        Break[];];];];
  For[i = 1, i <= len, i++, {sign, k, l} = VariableInterpretation[ISP[[i]]];
...
GenerateBasis[op_] := 
  ...
  Print["Cut equations for ISP are listed in the variable 'CutEqnISP'"];
  Basis = {};
  SpuriousBasis = {};
  NSpuriousBasis = {};
  TempMonoList = {};
  ...
  (*LTList=Table[MonomialList[Gr[[i]],ISP,PolyOrder][[1]],{i,1,Length[Gr]}];*)
  TargetPower = Table[0, {i, 1, len}];
  StopSign = 0;
  If[Gr == {1}, StopSign = 1];
  While[StopSign == 0,
  (*TargetPower=IntegerDigits[i,(ISPPowerUpLimit+1),len];*)
  (*If[i==Floor[(ISPPowerUpLimit+1)^len*k/100],Print[k/100*100,"%     ",TimeUsed[]-ttt];k++];*)
    If[RenorTest[TargetPower] == 1, TargetCount++;
      r = 
        PolynomialReduce[
          ExpRecover[TargetPower, ISP], 
          Gr, 
          ISP, 
          MonomialOrder -> PolyOrder
        ][[2]];
    ...
    TempExpList = 
      Table[ExpRead[TempMonoList[[j]], ISP], {j, 1, Length[TempMonoList]}];
    If[r == 0, TempExpList = {}];
    ...
    Print[];
    Print["The basis contains ", Length[Basis], " terms, which are listed in the variable 'Basis'"];
    Print["The explicit form of the integrand is listed in the variable 'Integrand'"];
    Print["Number of spurious terms: ", SpuriousCount, ", listed in the variable 'SpuriousBasis'"];
    Print["Number of non-spurious terms: ", Length[Basis] - SpuriousCount, ", listed in the variable 'NSpuriousBasis'"];
